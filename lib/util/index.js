...
module.exports = {};
module.exports.retryable = async(page, settings, action, retryWithReload = true) => {
  const reloadAction = async() => page.reload()
  
  return promiseRetry(async(retry, attempt) => {
    console.log(`Making a request to the server. Attempt number ${attempt}`);

    const localAction = retryWithReload && attempt > 1 ? reloadAction : action 
