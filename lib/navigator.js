...
module.exports = class Navigator {
  constructor(browser, page, settings, account, tracker) {
    this.states = {
      ...
      confirmSchedulingState: new states.ConfirmSchedulingState(),
      billingState: new states.BillingState()
    };
  ...
  async handleCaptcha(state) {
    return this.captchaPromise
      .then(async(buffer) => util.resolveCaptcha(this.settings, buffer))
      .catch(async(err) => {
        console.log("Error trying to solve the captcha. Trying again.");
        console.error(err);

        await state.navigate(this, this.page, this.settings, this.page.reload(), false);
